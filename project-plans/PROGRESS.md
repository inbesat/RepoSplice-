...
**Current Phase:** P-020 — core: tree-sitter + grammars (in progress)
...
**P-020 (🔄 active):** Add `web-tree-sitter` + tree-sitter WASM grammars (TypeScript, JavaScript, Python, Go, Rust) to `packages/core`. Create `packages/core/src/analysis/treeSitter.ts` with loader + parse helpers. Used by P-148/P-152 for syntax-aware file selection and by P-104/105/106/107 for import/usage scanning.

**Defer to P-021 (next after P-020):** core dep `ignore` (P-104 file walker).
