...
**Current Phase:** P-060 — Root dev dep: vitest + @vitest/ui (awaiting go-ahead)
...
**P-060 (⏳ next, awaiting go-ahead):** root dev dep `vitest` + `@vitest/ui` per PHASES_DETAILED.md P-060 (read the FULL spec first; verify real exports/versions against the installed packages before wiring scripts). NOTE: root-level install (`bun add -d --cwd <root>`); never `--filter`.

**Defer to P-061 (next after P-060):** per plan sequence (check PHASES_DETAILED.md P-061).
