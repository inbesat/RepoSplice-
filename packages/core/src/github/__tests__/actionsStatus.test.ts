...
describe('errors', () => {
  it('maps endpoint failures with operation context', async () => {
    ...
    expect(listed.isErr()).toBe(true);
    if (listed.isOk()) return;
    expect(listed.error.code).toBe('AUTH_FAILED');
    if (listed.error.code !== 'AUTH_FAILED') return;
  ...
  it('reads every rate-limit shape on run calls', async () => {
    ...
    expect(remaining.error.code).toBe('RATE_LIMIT');
    ...
    expect(nestedResult.error.code).toBe('RATE_LIMIT');
    if (nestedResult.error.code !== 'RATE_LIMIT') return;
